...
namespace Sc.Packet
{
    ...
    public class LocalApiService : IApiService
    {
        ...
        private const int SimulatedLatencyMs = 100;
        ...
        public string CurrentUserId => _userData.Profile.Uid;
        ...
        public async UniTask<bool> InitializeAsync()
        {
            ...
            await UniTask.Delay(SimulatedLatencyMs);
            ...
            if (File.Exists(SaveFilePath))
            {
                try
                {
                    ...
                    Debug.Log($"[LocalApiService] 저장 데이터 로드 완료: {_userData.Profile.Nickname}");
                }
                catch (Exception e)
                {
                    Debug.LogWarning($"[LocalApiService] 저장 데이터 로드 실패: {e.Message}");
                    _userData = default;
                }
            }
        ...
        public async UniTask<LoginResponse> LoginAsync(LoginRequest request)
        {
            ...
            if (string.IsNullOrEmpty(_userData.Profile.Uid))
            {
                ...
                // 초기 캐릭터 지급 (튜토리얼 캐릭터)
                _userData.Characters.Add(OwnedCharacter.Create("char_005")); // 록 (R등급 탱커)

                await SaveUserDataAsync();
                Debug.Log($"[LocalApiService] 신규 유저 생성: {nickname}");
            }
            else
            {
                // 기존 유저 로그인
                ...
                await SaveUserDataAsync();
                Debug.Log($"[LocalApiService] 기존 유저 로그인: {_userData.Profile.Nickname}");
            }
        ...
        public async UniTask<UserSaveData> FetchUserDataAsync()
        {
            await UniTask.Delay(SimulatedLatencyMs);
            return _userData;
        }
        ...
        public async UniTask<ShopPurchaseResponse> PurchaseAsync(ShopPurchaseRequest request)
        {
            await UniTask.Delay(SimulatedLatencyMs);
            ...
            if (request.ProductId == "gold_pack_small")
            {
                ...
                // 재화 차감 및 지급
                _userData.Currency.FreeGem -= Math.Min(gemCost, _userData.Currency.FreeGem);
                if (gemCost > _userData.Currency.FreeGem)
                {
                    _userData.Currency.Gem -= (gemCost - _userData.Currency.FreeGem);
                }
            ...
            where TResponse : IResponse
        {
            ...
            // 범용 요청 처리 (확장용)
            throw new NotImplementedException($"SendAsync not implemented for {typeof(TRequest).Name}");
        }
        ...
        private async UniTask SaveUserDataAsync()
        {
            try
            {
                ...
                Debug.Log($"[LocalApiService] 데이터 저장 완료");
            }
            catch (Exception e)
            {
                Debug.LogError($"[LocalApiService] 데이터 저장 실패: {e.Message}");
            }
        }
